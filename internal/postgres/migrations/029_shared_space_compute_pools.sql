CREATE TABLE shared_space_compute_pools (
    ...
    PRIMARY KEY (space_id, pool_id)
);

CREATE TABLE shared_space_compute_pool_changes (
    space_id uuid NOT NULL REFERENCES shared_spaces(space_id) ON DELETE CASCADE,
    ...
    FOREIGN KEY (space_id, pool_id)
        REFERENCES shared_space_compute_pools(space_id, pool_id) ON DELETE CASCADE
);
...
CREATE INDEX shared_space_compute_pool_changes_pool_idx
    ON shared_space_compute_pool_changes (space_id, pool_id, changed_at_milliseconds);
